 ...
 * @author Cesar
 */
public class AddProductRedirect extends HttpServlet {
        ...
        try 
        {
            if (con != null) 
            {
                ...
                ResultSet records = stmt.executeQuery("SELECT * FROM ITEM "
                        ...
                        + "WHERE DISABLED = FALSE AND GEN_NAME = 'Food Item' ORDER BY ITEM_NUM");
                
                request.setAttribute("items", records);
                request.getRequestDispatcher("p-addProduct.jsp").forward(request,response);
